...
<title>Добавить к проекту</title>
</head>
<body>
...
<? include 'navigation_bar.php';?>
  <hr>
    <p class="text-justify category-p">Добавить к проекту: <? echo $id?></p>
      <hr>
          ...
          <div class="material-box">
            ...
            <div class="form-group">
              ...
              <div class="input-group">
                ...
                <input type="file" class="custom-file-input file_input" id="imageFile" name="imageFile" onchange="validateFile(this, ['png', 'jpg','hdr','exr','EXR']); updateLabel(this)" multiple>
            ...
            <input type="hidden" class="form-control file_input" id="id" name="id"   value="<?php echo $id; ?>">
...
function clearFileInput(button) {
  ...
  function getValidFileTypes(type) {
    ...
    switch (type) {
      case 'image':
        return ['png', 'jpg','.hdr','HDR','exr','EXR'];
      case 'texture':
        return ['rar', 'zip'];
      case 'preview':
        return ['png', 'jpg'];
...
document.getElementById('uploadForm').addEventListener('submit', async function(event) {
    ...
    try {
            const response = await fetch('adding_to_project.php', {
            method: 'POST',
            body: formData,
                 });
                if (response.ok) {
...
function prepareModel(loadedObject, extension) {
    ...
    model = new THREE.Mesh(
        loadedObject, // Это BufferGeometry
...
<script>
  ...
  document.getElementById('modelInput').addEventListener('change', function() {
    ...
    var labelText = 'Выбрано файлов: ' + files.length;
