...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if ($user) {
            $userId = $user->id;
            $token = bin2hex(random_bytes(16)) . '-' . $userId . '-' . bin2hex(random_bytes(16));
            $user->recovery_token = $token;
            R::store($user);
            if (sendPasswordRecovery($email, $token)) {
                echo '<script>alert("Письмо отправлено успешно.");</script>';
            } else {
                echo '<script>alert("Ошибка при отправке письма.");</script>';
            }
        } else {
            echo '<script>alert("Указанный адрес электронной почты не найден.");</script>';
        }
    } catch (Exception $e) {
...
function sendPasswordRecovery($email, $token) {
  ...
  try {
      ...
      $mail->SMTPAuth = true;
      $mail->Username = 'geraldine.daugherty97@ethereal.email';
      $mail->Password = 'bjSpgNTQD1JjXxGH34';
      $mail->SMTPSecure = 'tls';
      $mail->Port = 587;
      $mail->CharSet = 'UTF-8';
      // Отправитель и получатель
      $mail->setFrom('mail_email@example.com', 'Admin');
      ...
      $mail->isHTML(true);
      $mail->Body = '
      <div style="max-width: 500px; margin: auto; font-family: Arial, sans-serif; border: 1px solid #ddd; border-radius: 10px; padding: 20px; background-color: #f9f9f9;">
          <h2 style="text-align: center; color: #333;">🔑 Восстановление пароля</h2>
          <p style="font-size: 16px; color: #555;">Вы запросили восстановление пароля. Нажмите на кнопку ниже, чтобы сбросить его:</p>
          <div style="text-align: center; margin: 20px 0;">
              <a href="http://three.js-dev/password_change.php?token=' . $token . '" 
                 style="display: inline-block; padding: 10px 20px; font-size: 18px; color: #fff; background: #007BFF; text-decoration: none; border-radius: 5px;">
                 🔄 Сбросить пароль
              </a>
          </div>
          <p style="font-size: 14px; color: #999;">Если вы не запрашивали сброс пароля, просто проигнорируйте это письмо.</p>
          <hr style="border: none; border-top: 1px solid #ddd;">
          <p style="text-align: center; font-size: 12px; color: #777;">&copy; ' . date("Y") . '  «Учебно-производственный центр» (УПЦ) ООО «Газпром трансгаз Ухта». Все права защищены.</p>
      </div>';
      return $mail->send();
  } catch (Exception $e) {
  ...
  <div class="d-flex md-12">
    ...
    <div class="container rounded-container">
        <div class="row justify-content-center">
            <div class="col-md-6">
                <H1 style="color:white;">Забыли пароль:</H1>
                <form method="post" action="forgot.php">
                    <div class="form-group text-center">
                        <label for="email">Электронная почта:</label>
                        <input type="email" name="email" class="form-control" id="email" placeholder="Введите адрес электронной почты" required>
                        <?php echo '<button type="submit" class="btn btn-primary logout_btn" style="margin-top: 9px;">Восстановить</button>'?>
                        </div>
                    <div class="form-group text-center">
                    <?php echo '<a href="login.php" class="btn btn-link">Вернуться к входу</a>'?>
                    </div>
                    </form>
                </div>
...
<script>
  ...
  const rect = document.querySelector('#fillRect');
